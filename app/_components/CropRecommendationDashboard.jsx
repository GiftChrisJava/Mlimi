...
export default function CropRecommendationDashboard({ formData, city, onReset }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch('https://mlimi.onrender.com/api/v1/recommendations', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          state: formData.city,
          soilCondition: `soil temperature: ${formData.soilTemperature}, soil moisture: ${formData.soilMoisture}, soil pH: ${formData.soilPH}`,
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4 mr-[100px]">
      <div className="p-4 bg-white shadow-lg rounded-lg w-full max-w-2xl">
        <h2 className="text-xl font-semibold mb-4 text-center">Crop Recommendation Dashboard</h2>
        <h3 className="text-lg font-semibold mb-2">Location: {city}</h3>
        <p className="mb-4">{cropData.recommendation}</p> {/* Adjust to the correct key in cropData */}
        <button onClick={onReset} className="mt-4 w-full bg-blue-700 text-white px-4 py-2 rounded">Reset</button>
      </div>
    </div>
  );
}
...
//         <h2 className="text-xl font-semibold mb-4 text-center">Crop Recommendation Dashboard</h2>
//         <h3 className="text-lg font-semibold mb-2">Location: {city}</h3>
//         <p className="mb-4">{cropData.description}</p>
